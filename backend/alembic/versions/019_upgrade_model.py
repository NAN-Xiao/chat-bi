"""迁移脚本：019_upgrade_model

迁移版本 ID： dcaecd481715
上一版本： 863105882eba
创建时间： 2025-07-04 14:45:56.927188
"""
...
def upgrade():
    """
    是什么：upgrade 是 backend/alembic/versions/019_upgrade_model.py 中的同步数据库迁移函数。
...
def downgrade():
    # ### Alembic 自动生成的命令，请按需调整！###
    """
    是什么：downgrade 是 backend/alembic/versions/019_upgrade_model.py 中的同步数据库迁移函数。
    谁调用：由 Alembic 迁移框架在执行数据库升级或回滚时调用。

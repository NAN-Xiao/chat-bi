"""迁移脚本：117_refresh_saas_skill_windowed_lifecycle_rules

迁移版本 ID： b7c6d5e4f3a2
上一版本： c9d8e7f6a5b4
...
def _load_saas_skills_module():
    """
    是什么：_load_saas_skills_module 是 backend/alembic/versions/117_refresh_saas_skill_windowed_lifecycle_rules.py 中的同步函数。
...
def upgrade() -> None:
    """
    是什么：upgrade 是 backend/alembic/versions/117_refresh_saas_skill_windowed_lifecycle_rules.py 中的同步数据库迁移函数。
...
def downgrade() -> None:
    """
    是什么：downgrade 是 backend/alembic/versions/117_refresh_saas_skill_windowed_lifecycle_rules.py 中的同步数据库迁移函数。
    谁调用：由 Alembic 迁移框架在执行数据库升级或回滚时调用。

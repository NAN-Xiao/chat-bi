...
- 生命周期/时间序列结果缺少中间日期时，不要直接解释为数据采集遗漏或业务断点；只有查询已补齐日期序列且 rows 明确显示 0 或异常字段时，才可描述为无行为/异常。
- daily_revenue、daily_amount、daily_payers 等按日聚合字段只能描述“当日合计”或“当日付费用户”，除非 rows 明确包含 order_id 或 order_count=1，否则不要写成“单笔”。
- 判断“最高峰/最大值”必须比较当前结果中完整的相关行；如果只是在早期窗口内较高，应说“早期高点”，不要说成全周期峰值。
- 不要跨日期、商品、渠道、分层等维度简单相加去重人数、付费人数、客户数等 distinct 指标；只有查询结果已经提供整体去重字段或累计去重字段时，才给出总体人数。
"""
...
不要跨日期、商品、渠道、分层等维度简单相加去重人数、付费人数、客户数等 distinct 指标；只有 rows 中已经有整体去重字段、累计去重字段或明确可累加的金额/订单数时，才给出总体结论。
不要向业务用户提及 rows、summary、数据块截断、样例行、内部过程或“summary 明确指出”等实现细节；只输出业务结论和依据。
daily_revenue、daily_amount、daily_payers 等聚合字段只能描述“当日合计”或“当日付费用户”；除非 rows 明确包含 order_id 或 order_count=1，否则不要写成“单笔”。
...
def _lifecycle_cohort_validation_error(
    query: dict[str, Any],
    fields: list[str],
    rows: list[dict[str, Any]],
) -> str | None:
    text = _query_metric_text(query)
    if not any(keyword in text for keyword in ("生命周期", "后续付费", "ltv", "lifecycle", "cohort")):
        return None
    if not any(keyword in text for keyword in ("付费", "收入", "营收", "ltv", "revenue", "payment", "pay")):
        return None
    ...
    day_field = next(
        (
            field
            for key, field in lower_fields.items()
            if key
            in {
                "lifecycle_day",
                "x_lifecycle_day",
    ...
    if len(days) >= 2:
        ...
        if missing_days:
            ...
            return (
                f"生命周期趋势结果缺少连续日期 {sample}。"
                "这通常表示 SQL 只返回了有付费的 lifecycle_day。"
                "请使用 generate_series 或日期序列表补齐观察窗口，让无付费日期返回 0，并保持累计字段不下降。"
            )

    needs_ltv = any(keyword in text for keyword in ("ltv", "后续付费", "长期价值", "生命周期价值", "累积收入", "累计收入"))
    if needs_ltv and not any("ltv" in field.lower() for field in fields):
        return (
            "生命周期后续付费分析缺少 ltv 字段。"
            "请输出 daily_revenue、cumulative_revenue 和 ltv，"
            "其中 ltv = cumulative_revenue / 固定 cohort 人数。"
        )

    if any(keyword in text for keyword in ("累计", "累积", "cumulative", "后续付费", "累计付费")):
        has_cumulative_payers = any(
            any(keyword in field.lower() for keyword in ("cumulative_payer", "cumulative_paying", "cum_payer", "cum_paying"))
            for field in fields
        )
        if not has_cumulative_payers:
            return (
                "生命周期累计付费分析缺少累计去重付费用户字段。"
                "请先计算每个主体的首次成功付费生命周期日 first_pay_day，"
                "再按日期序列统计 first_pay_day <= lifecycle_day 的累计去重主体。"
            )
...
def _semantic_validation_error(query: dict[str, Any], result: dict[str, Any]) -> str | None:
    ...
    lifecycle_error = _lifecycle_cohort_validation_error(query, fields, rows)
    if lifecycle_error:

...
def _can_manage_platform_prompt_runtime(user: CurrentUser | None) -> bool:
    """
    是什么：_can_manage_platform_prompt_runtime 是一个可以复用的小步骤，负责分析助手相关的一件事。
...
def _can_manage_tenant_prompt_runtime(user: CurrentUser | None) -> bool:
    """
    是什么：_can_manage_tenant_prompt_runtime 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
class AnalysisAssistantMessage(BaseModel):
    """
    类说明：AnalysisAssistantMessage 把分析助手相关的数据和行为放在一起，便于其他代码直接复用。
    """
...
class AnalysisAssistantExportBlock(BaseModel):
    """
    类说明：AnalysisAssistantExportBlock 把分析助手相关的数据和行为放在一起，便于其他代码直接复用。
    """
...
class AnalysisAssistantExportRequest(BaseModel):
    """
    类说明：AnalysisAssistantExportRequest 用来描述分析助手的数据格式，让请求入参、返回结果和内部传值更清楚。
    """
    title: str | None = None
...
6. “当前图表/报表解读”的事实边界是当前图表可见数据，不是原始 SQL 可能查到的数据、隐藏行、未选中的分组、未激活 tab、Data Skill 示例或模型常识。

回答要求：
- 默认使用简体中文。
- 直接给最终解读，语气像看板旁边的业务分析师，简洁、明确、可执行。
- 固定只输出两个小节：`数据概览`、`分析建议`。
- `数据概览` 不是复述表格或图例；每条都必须包含“数据事实 + 判断/含义”，例如稳定、抬升、回落、异常、分化、低基数、样本不足、口径限制或与其它图表的印证/矛盾。
- `数据概览` 要综合当前目标内所有已提供图表，提炼核心数字、峰值、低点、变化方向、结构占比或图表间明显关联，最多 6 条；避免逐日期、逐国家、逐字段流水账。
- `分析建议` 必须对应前面的判断，说明为什么值得做、优先验证什么、预期能排除或确认什么；最多 5 条。
- 禁止把“关注、监控、核查、优化展示”当作万能建议。建议必须绑定当前可见数据中的具体信号；如果信号不足，只说明“当前可见数据不足以判断”，不要硬凑建议。
- 所有数字和趋势判断必须来自当前报表目标上下文或数据样例；数据不足时直接说明不能判断。
- 只能点名当前可见数据样例或可见维度/取值白名单里出现过的国家、渠道、日期、商品、服务器、分组等维度值；如果某个值没有出现在可见数据里，禁止引用、比较或建议排查它。
- 如果当前图表只显示一个分组，只能围绕这个分组分析；不要把隐藏分组、源结果中的其它分组或 SQL 可查询到的其它分组带入结论。
- 如果当前图表只显示一个分组，不要做“高价值市场/长尾市场/国家间对比”这类跨分组结论；应解释该分组自身趋势、波动、空值、样本限制和能/不能判断的范围。
...
def _to_langchain_messages(request: AnalysisAssistantRequest) -> list[BaseMessage]:
    """
    是什么：_to_langchain_messages 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _chunk_text(content) -> str:
    """
    是什么：_chunk_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if content is None:
...
async def _create_llm(custom_model_id: int | None = None) -> tuple[Any, LLMConfig]:
    """
    是什么：_create_llm 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
...
def _sse(payload: dict[str, Any]) -> str:
    """
    是什么：_sse 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _normalise_conversation_title(title: str | None, messages: list[dict]) -> str:
    """
    是什么：_normalise_conversation_title 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _serialise_conversation_messages(request: AnalysisAssistantConversationSave) -> list[dict]:
    """
    是什么：_serialise_conversation_messages 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    messages = []
...
def _conversation_summary(record: AnalysisAssistantConversation) -> AnalysisAssistantConversationSummary:
    """
    是什么：_conversation_summary 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
async def _tenant_rate_limit_response(session: SessionDep, current_user: CurrentUser):
    """
    是什么：_tenant_rate_limit_response 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    try:
...
def _trace(content: str, block_id: str | None = None) -> str:
    """
    是什么：_trace 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _mark_query_error_block(block: dict[str, Any], query_error: Exception, current_user: CurrentUser) -> None:
    """
    是什么：_mark_query_error_block 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _extract_json_object(text: str) -> dict[str, Any]:
    ...
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手的原始内容拆开、转换或整理，变成程序更好处理的格式。
    """
...
def _get_datasource(
    ...
    """
    是什么：_get_datasource 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
...
def _get_owned_conversation(
    ...
    *,
    ...
    """
    是什么：_get_owned_conversation 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手需要的数据找出来，整理成后面好用的样子。
    """
...
async def list_history(
    ...
    """
    是什么：list_history 是一个接口入口，负责接住分析助手相关请求。
...
async def get_history(conversation_id: int, current_user: CurrentUser, session: SessionDep):
    """
    是什么：get_history 是一个接口入口，负责接住分析助手相关请求。
    谁调用：前端或外部系统调用对应接口时，FastAPI 会把请求交给它。
    做了什么：把分析助手需要的数据找出来，整理成后面好用的样子。
    """
...
async def delete_history(conversation_id: int, current_user: CurrentUser, session: SessionDep):
    """
    是什么：delete_history 是一个接口入口，负责接住分析助手相关请求。
    谁调用：前端或外部系统调用对应接口时，FastAPI 会把请求交给它。
    做了什么：把分析助手不再需要的数据、缓存或临时内容清理掉。
    """
...
async def save_history(
    ...
    """
    是什么：save_history 是一个接口入口，负责接住分析助手相关请求。
    谁调用：前端或外部系统调用对应接口时，FastAPI 会把请求交给它。
    做了什么：创建或保存分析助手需要的东西，让后续流程能继续往下走。
    """
...
def _strip_markdown(value: str | None) -> str:
    """
    是什么：_strip_markdown 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    text = str(value or "")
...
def _export_paragraphs(value: str | None) -> list[str]:
    """
    是什么：_export_paragraphs 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _clean_filename(value: str) -> str:
    """
    是什么：_clean_filename 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手不再需要的数据、缓存或临时内容清理掉。
    """
...
def _decode_export_image(data_url: str | None) -> tuple[bytes, str] | None:
    """
    是什么：_decode_export_image 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _xml_text(value: str) -> str:
    """
    是什么：_xml_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _docx_paragraph(
    ...
    *,
    ...
    """
    是什么：_docx_paragraph 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    return (
...
def _docx_image(image_rid: str, image_index: int, image_bytes: bytes) -> str:
    """
    是什么：_docx_image 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    try:
...
def _build_export_docx(request: AnalysisAssistantExportRequest) -> bytes:
    """
    是什么：_build_export_docx 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：创建或保存分析助手需要的东西，让后续流程能继续往下走。
    """
...
def _load_export_font(size: int, *, bold: bool = False):
    """
    是什么：_load_export_font 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手需要的数据找出来，整理成后面好用的样子。
    """
...
def _wrap_export_text(draw, text: str, font, max_width: int) -> list[str]:
    """
    是什么：_wrap_export_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    lines: list[str] = []
...
def _build_export_pdf(request: AnalysisAssistantExportRequest) -> bytes:
    """
    是什么：_build_export_pdf 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：创建或保存分析助手需要的东西，让后续流程能继续往下走。
    """
    ...
    def footer() -> None:
        """
        是什么：footer 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _build_export_pdf 跑到对应步骤时会调用它。
        做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
        """
        page_no = len(pages) + 1
    ...
    def new_page() -> None:
        """
        是什么：new_page 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _build_export_pdf 跑到对应步骤时会调用它。
        做了什么：创建或保存分析助手需要的东西，让后续流程能继续往下走。
        """
    ...
    def ensure(height: int) -> None:
        """
        是什么：ensure 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _build_export_pdf 跑到对应步骤时会调用它。
        做了什么：检查分析助手里的数据、权限或配置是否合法，不对就及时拦住。
        """
    ...
    def write_text(text: str, font, fill: str = "#15233B", spacing: int = 8, after: int = 14) -> None:
        """
        是什么：write_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _build_export_pdf 跑到对应步骤时会调用它。
        做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
        """
        nonlocal y
    ...
    def write_image(data_url: str | None) -> None:
        """
        是什么：write_image 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _build_export_pdf 跑到对应步骤时会调用它。
        做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
        """
        nonlocal y
...
async def export_report(
    ...
    """
    是什么：export_report 是一个接口入口，负责接住分析助手相关请求。
    谁调用：前端或外部系统调用对应接口时，FastAPI 会把请求交给它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not current_user:
...
def _collect_custom_agent_context(
    ...
    """
    是什么：_collect_custom_agent_context 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _collect_data_skill_context(
    ...
    """
    是什么：_collect_data_skill_context 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    try:
...
def _data_skill_block(data_skill: str) -> str:
    """
    是什么：_data_skill_block 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _data_skill_final_system_rules(data_skill: str = "") -> str:
    """
    是什么：_data_skill_final_system_rules 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not data_skill or not data_skill.strip():
...
def _report_skill_block(data_skill: str) -> str:
    """
    是什么：_report_skill_block 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not data_skill or not data_skill.strip():
    ...
    return (
        ...
        "不得覆盖当前图表里已经给出的可见数据事实，不得引入当前可见样例或白名单以外的维度取值，"
        "不得要求重新查询数据，不得改变本接口固定输出格式）：\n"
        f"{data_skill[:24000]}\n\n"
    )
...
def _profile_result_as_text(title: str, result: dict[str, Any], limit: int = 80) -> str:
    """
    是什么：_profile_result_as_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _quote_identifier(datasource: CoreDatasource, identifier: str) -> str:
    """
    是什么：_quote_identifier 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _profile_table_expression(datasource: CoreDatasource, raw_table: str) -> tuple[str, str]:
    """
    是什么：_profile_table_expression 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _is_forecast_question(question: str) -> bool:
    """
    是什么：_is_forecast_question 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _is_number(value: Any) -> bool:
    """
    是什么：_is_number 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _numeric_fields(fields: list[str], rows: list[dict[str, Any]]) -> list[str]:
    """
    是什么：_numeric_fields 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    numeric = []
...
def _match_field(value: str | None, fields: list[str]) -> str | None:
    """
    是什么：_match_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not value:
...
def _field_matches(field: str | None, keywords: tuple[str, ...]) -> bool:
    """
    是什么：_field_matches 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not field:
...
def _query_metric_text(query: dict[str, Any]) -> str:
    """
    是什么：_query_metric_text 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手需要的数据找出来，整理成后面好用的样子。
    """
    return " ".join(
...
def _funnel_order_field(fields: list[str]) -> str | None:
    """
    是什么：_funnel_order_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    return next(
...
def _funnel_step_field(query: dict[str, Any], fields: list[str]) -> str | None:
    """
    是什么：_funnel_step_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _requested_metric_field(query: dict[str, Any], numeric: list[str], x_field: str | None) -> str | None:
    """
    是什么：_requested_metric_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _choose_metric_field(query: dict[str, Any], numeric: list[str], x_field: str | None) -> str | None:
    """
    是什么：_choose_metric_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not numeric:
...
def _looks_like_time_field(field: str | None) -> bool:
    """
    是什么：_looks_like_time_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _time_series_trend_requested(query: dict[str, Any]) -> bool:
    """
    是什么：_time_series_trend_requested 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    text = " ".join(
...
def _category_total_requested(query: dict[str, Any]) -> bool:
    """
    是什么：_category_total_requested 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    text = " ".join(
...
def _first_time_field(fields: list[str]) -> str | None:
    """
    是什么：_first_time_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _first_category_field(fields: list[str], numeric: list[str], exclude: set[str]) -> str | None:
    """
    是什么：_first_category_field 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _looks_like_metric_card(query: dict[str, Any], rows: list[dict[str, Any]]) -> bool:
    """
    是什么：_looks_like_metric_card 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _choose_visual_chart_type(
    ...
    """
    是什么：_choose_visual_chart_type 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if chart_type != "table":
...
def _explicit_donut_requested(query: dict[str, Any]) -> bool:
    """仅依据用户原问题判断是否明确要求环形图。"""
    text = str(query.get("_user_question") or "").lower()
    return any(keyword in text for keyword in ("环形图", "圆环图", "donut", "doughnut"))


def _prefers_pie_chart(query: dict[str, Any], rows: list[dict[str, Any]], x_field: str | None) -> bool:
    """
    是什么：_prefers_pie_chart 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _is_pie_metric_suitable(query: dict[str, Any], y_field: str | None) -> bool:
    """
    是什么：_is_pie_metric_suitable 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    text = _query_metric_text(query)
...
def _build_chart_config(query: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    """
    是什么：_build_chart_config 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：创建或保存分析助手需要的东西，让后续流程能继续往下走。
    """
    ...
    if chart_type not in CHART_TYPES:
        chart_type = "table"
    if chart_type == "donut" and not _explicit_donut_requested(query):
        chart_type = "table"
...
def _compact_rows(rows: list[dict[str, Any]], limit: int = 30) -> str:
    """
    是什么：_compact_rows 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _coerce_float(value: Any) -> float | None:
    """
    是什么：_coerce_float 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if value is None or value == "":
...
def _coerce_day_number(value: Any) -> int | None:
    """
    是什么：_coerce_day_number 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if value is None:
...
def _normalize_skill_list(value: Any) -> list[str]:
    """
    是什么：_normalize_skill_list 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手的原始内容拆开、转换或整理，变成程序更好处理的格式。
    """
    if value in (None, ""):
...
def _skill_rule_matches_query(rule: dict[str, Any], query: dict[str, Any]) -> bool:
    """
    是什么：_skill_rule_matches_query 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _extract_data_skill_validation_rules(data_skill: str = "") -> list[dict[str, Any]]:
    """
    是什么：_extract_data_skill_validation_rules 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手的原始内容拆开、转换或整理，变成程序更好处理的格式。
    """
    if not data_skill or not data_skill.strip():
...
def _field_present_by_keywords(fields: list[str], keywords: list[str]) -> bool:
    """
    是什么：_field_present_by_keywords 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not keywords:
...
def _fields_by_keywords(fields: list[str], keywords: list[str]) -> list[str]:
    """
    是什么：_fields_by_keywords 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not keywords:
...
def _format_skill_validation_message(template: Any, fallback: str, **kwargs: Any) -> str:
    """
    是什么：_format_skill_validation_message 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手的原始内容拆开、转换或整理，变成程序更好处理的格式。
    """
    if not template:
...
def _skill_declared_validation_error(
    ...
    """
    是什么：_skill_declared_validation_error 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
...
def _value_range_error(fields: list[str], rows: list[dict[str, Any]]) -> str | None:
    """
    是什么：_value_range_error 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    rate_keywords = (
...
def _wide_funnel_validation_error(
    ...
    """
    是什么：_wide_funnel_validation_error 是一个可以复用的小步骤，负责分析助手相关的一件事。
    谁调用：同一个接口脚本里的路由函数或辅助逻辑会调用它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    text = _query_metric_text(query)
...
def _final_answer_messages(
    ...
    *,
    ...
    def rows_for_final(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        是什么：rows_for_final 是一个可以复用的小步骤，负责分析助手相关的一件事。
        谁调用：外层函数 _final_answer_messages 跑到对应步骤时会调用它。
        做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
        """
        if len(rows) <= 50:
            return rows
...
async def chat(request: AnalysisAssistantRequest, current_user: CurrentUser, session: SessionDep):
    ...
    def generate():
        """
        是什么：generate 是一个可以复用的小步骤，负责分析助手相关的一件事。
...
async def report_interpretation(request: AnalysisAssistantRequest, current_user: CurrentUser, session: SessionDep):
    """
    是什么：report_interpretation 是一个接口入口，负责接住分析助手相关请求。
    谁调用：前端或外部系统调用对应接口时，FastAPI 会把请求交给它。
    做了什么：把分析助手里这一步需要处理的内容整理好，交给后面的代码继续用。
    """
    if not current_user:

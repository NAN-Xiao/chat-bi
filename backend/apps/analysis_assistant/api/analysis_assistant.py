...
- 每个图表/数据块都要给一句业务总结。
- 最终答案要包含结论和可执行建议。
- 业务分析口径必须来自当前项目/数据源已配置的术语和 SQL 示例，或来自用户本次明确给出的规则；不要把行业经验、示例数据、历史回答或代码里的隐含规则当作平台口径。"""
...
- 具体指标定义、业务口径和标准算法以项目已配置的术语与 SQL 示例为准；如果缺少口径配置，只能说明需要补充平台管理中的术语或 SQL 示例，不要自行固化业务算法。
...
- 具体指标定义、字段选择、计算算法、时间窗口和异常判断必须严格遵循“统一业务口径”的术语与 SQL 示例，或用户本次明确给出的规则。
- 如果统一业务口径没有覆盖某个业务指标、分母、表/字段选择、时间窗口或判断规则，不要把 schema、样例数据、行业经验或历史回答推断成确定口径；应在计划中说明需要补充平台管理中的术语或 SQL 示例，或只生成不依赖该缺失业务口径的基础探索查询。
...
- 具体预测算法、字段选择、指标口径和行业定义必须严格遵循“统一业务口径”的术语与 SQL 示例，或用户本次明确给出的规则；不要在提示词中写死某一个业务指标的算法。
- 如果预测目标缺少统一业务口径，不要自行发明预测算法或分母定义；应说明需要在平台管理中补充术语或 SQL 示例，或只输出数据不足/口径不足的原因。

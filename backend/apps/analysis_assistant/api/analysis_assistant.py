...
def _can_manage_platform_prompt_runtime(user: CurrentUser | None) -> bool:
    """
    是什么：_can_manage_platform_prompt_runtime 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _can_manage_platform_prompt_runtime 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _can_manage_tenant_prompt_runtime(user: CurrentUser | None) -> bool:
    """
    是什么：_can_manage_tenant_prompt_runtime 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _can_manage_tenant_prompt_runtime 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
SYSTEM_PROMPT = """你是星通数智内置的综合分析助手，一个独立于“智能报表”的业务分析 Agent。
...
REPORT_INTERPRETATION_PROMPT = """你是星通数智的默认“报表解读”Agent，专门解读看板里当前选中的报表区域或整张看板。
...
def _to_langchain_messages(request: AnalysisAssistantRequest) -> list[BaseMessage]:
    """
    是什么：_to_langchain_messages 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _to_langchain_messages 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _chunk_text(content) -> str:
    """
    是什么：_chunk_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _chunk_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if content is None:
        return ""
...
async def _create_llm(custom_model_id: int | None = None) -> tuple[Any, LLMConfig]:
    """
    是什么：_create_llm 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
...
def _sse(payload: dict[str, Any]) -> str:
    """
    是什么：_sse 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _sse 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _current_tenant_id(current_user: CurrentUser) -> int:
    """
    是什么：_current_tenant_id 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _current_tenant_id 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _normalise_conversation_title(title: str | None, messages: list[dict]) -> str:
    """
    是什么：_normalise_conversation_title 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _normalise_conversation_title 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _serialise_conversation_messages(request: AnalysisAssistantConversationSave) -> list[dict]:
    """
    是什么：_serialise_conversation_messages 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _serialise_conversation_messages 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    messages = []
...
def _conversation_summary(record: AnalysisAssistantConversation) -> AnalysisAssistantConversationSummary:
    """
    是什么：_conversation_summary 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _conversation_summary 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _conversation_detail(record: AnalysisAssistantConversation) -> AnalysisAssistantConversationDetail:
    """
    是什么：_conversation_detail 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _conversation_detail 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _rate_limit_message(retry_after_seconds: int) -> str:
    """
    是什么：_rate_limit_message 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _rate_limit_message 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _quota_message(state) -> str:
    """
    是什么：_quota_message 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _quota_message 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
async def _tenant_rate_limit_response(session: SessionDep, current_user: CurrentUser):
    """
    是什么：_tenant_rate_limit_response 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _tenant_rate_limit_response 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    try:
...
def _trace(content: str, block_id: str | None = None) -> str:
    """
    是什么：_trace 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _trace 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _mark_query_error_block(block: dict[str, Any], query_error: Exception, current_user: CurrentUser) -> None:
    """
    是什么：_mark_query_error_block 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _mark_query_error_block 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _llm_text(llm, messages: list[BaseMessage]) -> str:
    """
    是什么：_llm_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _llm_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _extract_json_object(text: str) -> dict[str, Any]:
    """
    是什么：_extract_json_object 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：解析、转换或格式化分析助手相关数据，生成后续流程可使用的结构。
    """
...
def _normalise_sql(sql: str) -> str:
    """
    是什么：_normalise_sql 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _normalise_sql 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    sql = (sql or "").strip()
...
def _get_datasource(
    ...
    """
    是什么：_get_datasource 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
...
def _get_owned_conversation(
    ...
    *,
    ...
    """
    是什么：_get_owned_conversation 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
...
async def list_history(
    ...
    """
    是什么：list_history 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
...
async def get_history(conversation_id: int, current_user: CurrentUser, session: SessionDep):
    """
    是什么：get_history 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
...
async def delete_history(conversation_id: int, current_user: CurrentUser, session: SessionDep):
    """
    是什么：delete_history 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：删除或清理分析助手相关数据、缓存或临时状态。
    """
...
async def save_history(
    ...
    """
    是什么：save_history 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
...
def _strip_markdown(value: str | None) -> str:
    """
    是什么：_strip_markdown 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _strip_markdown 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    text = str(value or "")
    if not text.strip():
        return ""
...
def _export_paragraphs(value: str | None) -> list[str]:
    """
    是什么：_export_paragraphs 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _export_paragraphs 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _clean_filename(value: str) -> str:
    """
    是什么：_clean_filename 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：删除或清理分析助手相关数据、缓存或临时状态。
    """
...
def _decode_export_image(data_url: str | None) -> tuple[bytes, str] | None:
    """
    是什么：_decode_export_image 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _decode_export_image 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _xml_text(value: str) -> str:
    """
    是什么：_xml_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _xml_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _docx_paragraph(
    ...
    *,
    ...
    """
    是什么：_docx_paragraph 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _docx_paragraph 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    return (
...
def _docx_image(image_rid: str, image_index: int, image_bytes: bytes) -> str:
    """
    是什么：_docx_image 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _docx_image 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    try:
...
def _build_export_docx(request: AnalysisAssistantExportRequest) -> bytes:
    """
    是什么：_build_export_docx 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
...
def _load_export_font(size: int, *, bold: bool = False):
    """
    是什么：_load_export_font 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
...
def _wrap_export_text(draw, text: str, font, max_width: int) -> list[str]:
    """
    是什么：_wrap_export_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _wrap_export_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    lines: list[str] = []
...
def _build_export_pdf(request: AnalysisAssistantExportRequest) -> bytes:
    """
    是什么：_build_export_pdf 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
    ...
    def footer() -> None:
        """
        是什么：footer 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _build_export_pdf 在执行内部流程时调用。
        做了什么：围绕 footer 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
        """
        page_no = len(pages) + 1
    ...
    def new_page() -> None:
        """
        是什么：new_page 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _build_export_pdf 在执行内部流程时调用。
        做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
        """
    ...
    def ensure(height: int) -> None:
        """
        是什么：ensure 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _build_export_pdf 在执行内部流程时调用。
        做了什么：校验分析助手相关输入、权限、配置或运行状态，不满足条件时返回失败或抛出异常。
        """
    ...
    def write_text(text: str, font, fill: str = "#15233B", spacing: int = 8, after: int = 14) -> None:
        """
        是什么：write_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _build_export_pdf 在执行内部流程时调用。
        做了什么：围绕 write_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
        """
        nonlocal y
    ...
    def write_image(data_url: str | None) -> None:
        """
        是什么：write_image 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _build_export_pdf 在执行内部流程时调用。
        做了什么：围绕 write_image 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
        """
        nonlocal y
...
async def export_report(
    ...
    """
    是什么：export_report 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：围绕 export_report 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not current_user:
...
def _prepare_sql_for_execution(
    ...
    """
    是什么：_prepare_sql_for_execution 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _prepare_sql_for_execution 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _collect_custom_agent_context(
    ...
    """
    是什么：_collect_custom_agent_context 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _collect_custom_agent_context 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _collect_data_skill_context(
    ...
    """
    是什么：_collect_data_skill_context 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _collect_data_skill_context 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    try:
        tenant_id = _current_tenant_id(current_user) if current_user is not None else None
...
def _custom_agent_block(custom_agent: str) -> str:
    """
    是什么：_custom_agent_block 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _custom_agent_block 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _data_skill_block(data_skill: str) -> str:
    """
    是什么：_data_skill_block 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _data_skill_block 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _context_blocks(custom_agent: str = "", data_skill: str = "") -> str:
    """
    是什么：_context_blocks 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _context_blocks 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _custom_agent_final_system_rules(custom_agent: str = "") -> str:
    """
    是什么：_custom_agent_final_system_rules 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _custom_agent_final_system_rules 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not custom_agent or not custom_agent.strip():
        return ""
    return (
...
def _data_skill_final_system_rules(data_skill: str = "") -> str:
    """
    是什么：_data_skill_final_system_rules 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _data_skill_final_system_rules 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not data_skill or not data_skill.strip():
        return ""
    return (
...
def _report_skill_block(data_skill: str) -> str:
    """
    是什么：_report_skill_block 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _report_skill_block 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not data_skill or not data_skill.strip():
        return ""
    return (
...
def _report_interpretation_messages(
    ...
    """
    是什么：_report_interpretation_messages 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _report_interpretation_messages 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _stream_report_interpretation(
    ...
    """
    是什么：_stream_report_interpretation 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：组织分析助手的流式输出或异步等待，把事件和结果传递给调用方。
    """
    success = False
    try:
        if data_skill.strip():
...
def _profile_result_as_text(title: str, result: dict[str, Any], limit: int = 80) -> str:
    """
    是什么：_profile_result_as_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _profile_result_as_text 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _quote_identifier(datasource: CoreDatasource, identifier: str) -> str:
    """
    是什么：_quote_identifier 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _quote_identifier 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _profile_table_expression(datasource: CoreDatasource, raw_table: str) -> tuple[str, str]:
    """
    是什么：_profile_table_expression 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _profile_table_expression 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _collect_date_bounds(
    ...
    """
    是什么：_collect_date_bounds 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _collect_date_bounds 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _get_data_profile(
    ...
    """
    是什么：_get_data_profile 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
...
def _is_forecast_question(question: str) -> bool:
    """
    是什么：_is_forecast_question 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _is_forecast_question 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _is_number(value: Any) -> bool:
    """
    是什么：_is_number 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _is_number 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _numeric_fields(fields: list[str], rows: list[dict[str, Any]]) -> list[str]:
    """
    是什么：_numeric_fields 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _numeric_fields 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    numeric = []
    for field in fields:
...
def _match_field(value: str | None, fields: list[str]) -> str | None:
    """
    是什么：_match_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _match_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not value:
        return None
...
def _field_matches(field: str | None, keywords: tuple[str, ...]) -> bool:
    """
    是什么：_field_matches 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _field_matches 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not field:
        return False
    lowered = field.lower()
...
def _query_metric_text(query: dict[str, Any]) -> str:
    """
    是什么：_query_metric_text 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：读取或查询分析助手相关数据，整理后返回给调用方。
    """
    return " ".join(
...
def _funnel_order_field(fields: list[str]) -> str | None:
    """
    是什么：_funnel_order_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _funnel_order_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    return next(
        (
            field
...
def _funnel_step_field(query: dict[str, Any], fields: list[str]) -> str | None:
    """
    是什么：_funnel_step_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _funnel_step_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _requested_metric_field(query: dict[str, Any], numeric: list[str], x_field: str | None) -> str | None:
    """
    是什么：_requested_metric_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _requested_metric_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _choose_metric_field(query: dict[str, Any], numeric: list[str], x_field: str | None) -> str | None:
    """
    是什么：_choose_metric_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _choose_metric_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not numeric:
        return None
...
def _looks_like_time_field(field: str | None) -> bool:
    """
    是什么：_looks_like_time_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _looks_like_time_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _time_series_trend_requested(query: dict[str, Any]) -> bool:
    """
    是什么：_time_series_trend_requested 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _time_series_trend_requested 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    text = " ".join(
...
def _category_total_requested(query: dict[str, Any]) -> bool:
    """
    是什么：_category_total_requested 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _category_total_requested 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    text = " ".join(
        str(query.get(key) or "")
...
def _first_time_field(fields: list[str]) -> str | None:
    """
    是什么：_first_time_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _first_time_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _first_category_field(fields: list[str], numeric: list[str], exclude: set[str]) -> str | None:
    """
    是什么：_first_category_field 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _first_category_field 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _looks_like_metric_card(query: dict[str, Any], rows: list[dict[str, Any]]) -> bool:
    """
    是什么：_looks_like_metric_card 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _looks_like_metric_card 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _choose_visual_chart_type(
    ...
    """
    是什么：_choose_visual_chart_type 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _choose_visual_chart_type 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if chart_type != "table":
        return chart_type
...
def _prefers_pie_chart(query: dict[str, Any], rows: list[dict[str, Any]], x_field: str | None) -> bool:
    """
    是什么：_prefers_pie_chart 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _prefers_pie_chart 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _is_pie_metric_suitable(query: dict[str, Any], y_field: str | None) -> bool:
    """
    是什么：_is_pie_metric_suitable 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _is_pie_metric_suitable 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    text = _query_metric_text(query)
    if any(
        keyword in text
...
def _build_chart_config(query: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    """
    是什么：_build_chart_config 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
...
def _compact_rows(rows: list[dict[str, Any]], limit: int = 30) -> str:
    """
    是什么：_compact_rows 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _compact_rows 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _coerce_float(value: Any) -> float | None:
    """
    是什么：_coerce_float 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _coerce_float 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if value is None or value == "":
        return None
    try:
...
def _coerce_day_number(value: Any) -> int | None:
    """
    是什么：_coerce_day_number 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _coerce_day_number 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if value is None:
        return None
    if isinstance(value, (int, float)):
...
def _normalize_skill_list(value: Any) -> list[str]:
    """
    是什么：_normalize_skill_list 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：解析、转换或格式化分析助手相关数据，生成后续流程可使用的结构。
    """
    if value in (None, ""):
...
def _skill_rule_matches_query(rule: dict[str, Any], query: dict[str, Any]) -> bool:
    """
    是什么：_skill_rule_matches_query 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _skill_rule_matches_query 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _extract_data_skill_validation_rules(data_skill: str = "") -> list[dict[str, Any]]:
    """
    是什么：_extract_data_skill_validation_rules 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：解析、转换或格式化分析助手相关数据，生成后续流程可使用的结构。
    """
    if not data_skill or not data_skill.strip():
...
def _field_present_by_keywords(fields: list[str], keywords: list[str]) -> bool:
    """
    是什么：_field_present_by_keywords 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _field_present_by_keywords 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not keywords:
        return True
    return any(
...
def _fields_by_keywords(fields: list[str], keywords: list[str]) -> list[str]:
    """
    是什么：_fields_by_keywords 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _fields_by_keywords 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not keywords:
        return []
    return [
...
def _format_skill_validation_message(template: Any, fallback: str, **kwargs: Any) -> str:
    """
    是什么：_format_skill_validation_message 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：解析、转换或格式化分析助手相关数据，生成后续流程可使用的结构。
    """
    if not template:
...
def _skill_declared_validation_error(
    ...
    """
    是什么：_skill_declared_validation_error 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _skill_declared_validation_error 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _value_range_error(fields: list[str], rows: list[dict[str, Any]]) -> str | None:
    """
    是什么：_value_range_error 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _value_range_error 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    rate_keywords = (
        "rate",
        "ratio",
...
def _wide_funnel_validation_error(
    ...
    """
    是什么：_wide_funnel_validation_error 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _wide_funnel_validation_error 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    text = _query_metric_text(query)
...
def _semantic_validation_error(query: dict[str, Any], result: dict[str, Any], data_skill: str = "") -> str | None:
    """
    是什么：_semantic_validation_error 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _semantic_validation_error 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    rows = result.get("data") or []
    fields = [str(field) for field in result.get("fields") or []]
...
def _repair_sql(
    ...
    """
    是什么：_repair_sql 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _repair_sql 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    prompt = (
        f"用户问题：{question}\n"
...
def _summarise_block(
    ...
    """
    是什么：_summarise_block 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _summarise_block 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
...
def _final_answer_messages(
    ...
    """
    是什么：_final_answer_messages 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _final_answer_messages 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    block_details = []
    ...
    def rows_for_final(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
        是什么：rows_for_final 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 _final_answer_messages 在执行内部流程时调用。
        做了什么：围绕 rows_for_final 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
        """
        if len(rows) <= 50:
            return rows
...
def _final_answer(
    ...
    """
    是什么：_final_answer 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：围绕 _final_answer 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    return _llm_text(
        llm,
...
def _initial_outline_messages(
    ...
    """
    是什么：_initial_outline_messages 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
    question = request.messages[-1].content.strip()
...
def _build_plan(
    ...
    """
    是什么：_build_plan 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
    question = request.messages[-1].content.strip()
...
def _build_forecast_plan(
    ...
    """
    是什么：_build_forecast_plan 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
    谁调用：由 FastAPI 路由处理函数或同模块业务辅助流程调用。
    做了什么：创建、初始化或组装分析助手相关对象和数据，并返回或写入对应状态。
    """
    question = request.messages[-1].content.strip()
...
async def chat(request: AnalysisAssistantRequest, current_user: CurrentUser, session: SessionDep):
    """
    是什么：chat 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：围绕 chat 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not current_user:
        raise RuntimeError("Unauthorized")
    ...
    def generate():
        """
        是什么：generate 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的同步函数。
        谁调用：由外层函数 chat 在执行内部流程时调用。
        做了什么：基于输入上下文生成分析助手相关结果，并保存或返回给调用方。
        """
        question = request.messages[-1].content.strip()
        blocks: list[dict[str, Any]] = []
        success = False
...
async def report_interpretation(request: AnalysisAssistantRequest, current_user: CurrentUser, session: SessionDep):
    """
    是什么：report_interpretation 是 backend/apps/analysis_assistant/api/analysis_assistant.py 中的异步 FastAPI 接口处理函数。
    谁调用：由 FastAPI 路由系统在匹配到对应 HTTP 请求时调用。
    做了什么：围绕 report_interpretation 的语义处理分析助手相关逻辑，并把结果返回或写入状态。
    """
    if not current_user:
        raise RuntimeError("Unauthorized")
    if not request.messages or not request.messages[-1].content.strip():

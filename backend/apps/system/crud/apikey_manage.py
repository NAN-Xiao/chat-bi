...
@cache(
    ...
    cacheName=CacheName.ASK_INFO,
    keyExpression="access_key",
    scope="tenant",
    tenantExpression="tenant_id",
    singleFlight=False,
)
async def get_api_key(session: SessionDep, access_key: str, tenant_id: int | None = None) -> ApiKeyModel | None:
    """
    ...
    if tenant_id is not None:
        query = query.where(ApiKeyModel.tenant_id == int(tenant_id))
...
async def clear_api_key_cache(access_key: str, session: SessionDep | None = None, tenant_id: int | None = None):
    """

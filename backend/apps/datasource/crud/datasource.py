...
def _dictionary_schema_from_workspace(
    ...
    tracking_config = project_tracking_config_for_ai_context(
        get_tracking_config(session, int(tenant_id), int(ds.id))
    )
...
def get_ai_table_schema(session: SessionDep, current_user: CurrentUser, ds: CoreDatasource, question: str,
    ...
    """
    # 保留参数契约供共享 SQL 上下文调用；事件字典不再按 Data Skill 投影进 schema。
    _ = data_skill_text
    conf = DatasourceConf(**json.loads(aes_decrypt(ds.configuration))) if ds.type != "excel" else get_engine_config()

...
def _dashboard_config_prompt(
    ...
    return "\n".join([
        ...
        "筛选属性范围：指标内筛选可使用当前事件的 tracking-property 事件属性，或字段对象明确提供的当前事件表公共属性。",
        "全局筛选只允许使用 context.filters 中提供的当前事件表公共属性；对于带 sourceField、jsonPath、expression 的 JSON 子字段，必须使用字段对象的 expression，不得改用其他表的同名字段。",
    ...
    if str(analysis_model or "event") == "property":
        ...
        structure_prompt = (
            ...
            "同时展示事件必须按 entity_id + behavior_date 聚合，并将 behavior_date 与 matched.behavior_date 关联；不得只按 cohort_date 关联，否则会漏掉第 1 日到第 7 日的回访事件。simultaneous_value 只汇总 matched 回访窗口内的记录，不得参与 cohort 分母或 period_offset 分桶。\n"
             + COHORT_MATURITY_RULE + "\n"
             + COHORT_INPUT_RULE + "\n"
             "错误示例：cohort 保留事件明细时间、事件ID或其他细粒度列后直接 JOIN behavior；这会破坏 cohort_date + entity_id + related_property 唯一粒度。\n"
             "正确示例：cohort_unique 先 SELECT DISTINCT cohort_date, entity_id, related_property，再与 behavior 明细关联；最终 cohort_size 只在该唯一 Cohort 集合上计算。\n"
             "留存函数示例：可以使用 retention_range_count/retention_range_sum 减少手写 day_N 逻辑；先把原始 YYYYMMDD 字段转换为 DATE，按 entity_id 聚合 "
             "retention_range_count(is_first, is_active, event_date, ARRAY[1,2,3,4,5,6,7], 'normal')，"
             "再使用 retention_range_sum 汇总每天的 Cohort 留存结果。\n"
             "原生函数结果也必须转换成当前 result_contract 要求的 cohort_date、cohort_size、day_0 到 day_7 固定列；"
             "不能直接返回数组或展开成长表，关联属性和 simultaneous_value 必须在函数输入或外层唯一 Cohort 粒度中保留。\n"
         )
        ...
        structure_prompt = (
            ...
            "错误示例：不能从 base_events 直接计算 total_entities；population 直接 FROM base_events 使用 COUNT(*) OVER (...) 会在事件明细行上重复计算主体总数，不能作为分母。"
            "错误示例：SELECT CASE ... END AS interval_order, CASE WHEN interval_order ... END AS interval_label；interval_label 不能在同一层读取 interval_order。\n"
            "正确示例：先用 entity_values 按日期、全部分组键和 entity_id 聚合，再由 population 从 entity_values 计算窗口分母；"
            "再由 bucket_numbers 生成 interval_order，最后由 bucketed_labels 从 bucket_numbers 读取 interval_order 生成 interval_label。"
            "任何依赖列都必须来自前一层 CTE，不能依赖同一 SELECT 刚定义的别名。\n"
            ...
            "示例配置仅为 metric.kind=count、interval.mode=discrete、一个分组、未启用同时展示。"
            "configured_event_rows 是示例输入关系，一行代表一条已按当前授权、日期范围、事件和筛选配置过滤的明细；"
            "distribution_date 已按当前时间粒度转换，entity_id 映射 distribution.entityField，group_1 映射当前配置的一个结果分组维度。"
            "这些名称不是业务表或业务字段，必须用当前配置与授权 schema 构造输入，不能直接查询示例表。\n"
            "```sql\n"
            "WITH entity_values AS (\n"
            "    SELECT distribution_date, entity_id, group_1,\n"
            "           COUNT(*) AS distribution_value\n"
            "    FROM configured_event_rows\n"
            ...
            "自动区间边界仍按窗口期全部 entity_values 统一计算；同时展示启用时按上述显式 LEFT JOIN 范式扩展。"
            "示例不提供默认业务口径，日期参数、JSON 表达式、类型转换和聚合函数必须服从当前配置及数据源方言。\n"
        )
...
def _node_finalize_response(state: DashboardManualChartGraphState) -> dict[str, Any]:
    ...
    if response.analysis_model == "property":
        ...
        response.result_config = {
            ...
            "metric_kind": str((distribution.get("metric") or {}).get("kind") or "count"),
        }

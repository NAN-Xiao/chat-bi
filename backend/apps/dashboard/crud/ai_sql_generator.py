...
def _normalize_manual_config(
    ...
    if analysis_model not in {"event", "retention", "funnel", "distribution", "interval", "path", "attribution"}:
...
def _deterministic_validate_manual_config(
    ...
    return DashboardAiSqlGenerateResponse(
        ...
        analysis_model=analysis_model if analysis_model in {"retention", "funnel", "distribution", "interval", "path", "attribution"} else "event",
    )
...
def _dashboard_config_prompt(
    ...
    return "\n".join([
        ...
        "指标内筛选 rules 是可选配置；没有 rules 或 rules 为空时不是配置缺失，不要要求补筛选条件，不要生成空 WHERE/AND/CASE 条件；只有 rules 里存在有效字段、操作符和值时才应用该筛选。",
         "只允许使用 manual-dashboard-context 里的 selectedFields/metrics/formulaMetrics/calculatedMetrics/groups/filters/retention/funnel/distribution/interval/path/attribution 字段信息生成 SQL；不要编造未提供字段。",
        *retention_rules,
        *funnel_rules,
        *distribution_rules,
        *interval_rules,
...
async def _async_node_generate_sql(state: DashboardManualChartGraphState) -> dict[str, Any]:
    ...
    response.analysis_model = analysis_model if analysis_model in {"retention", "funnel", "distribution", "interval", "path", "attribution"} else "event"
    validation = state.get("validation_result")
    if validation:

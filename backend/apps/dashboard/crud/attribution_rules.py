...
ATTRIBUTION_RULES = [
    ...
    "目标指标支持 count、sum、avg、max、min、count_distinct，必须严格使用 targetMetric.aggregation 和 targetMetric.metricField；count 统计事件次数，count_distinct 对所选字段去重计数，sum/avg/max/min 分别对所选字段求和、平均、最大值、最小值，不能静默改成其他聚合或字段。",
    ...
    "count/sum 的 attributed_value=SUM(target_value*linear_weight)，count 的每次目标 target_value=1，sum 的每次目标 target_value 为所选数值属性；avg/max/min/count_distinct 必须按所选聚合计算贡献，不能套用次数或求和公式。每个目标的分配权重之和为1；target_count 可另外 COUNT(DISTINCT target_id) 展示，不能替代目标指标或触发次数。",

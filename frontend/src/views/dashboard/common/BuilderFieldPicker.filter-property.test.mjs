...
assert.match(source, /filterPropertyLabels\?: Partial<Record<FilterPropertyTab, string>>/, '调用方可以覆盖筛选属性展示名称')
...
assert.match(source, /if \(activeTab\.value === 'all'\) return '暂无筛选属性'/, '全部标签空列表需要明确空状态')
assert.match(source, /return `暂无\$\{label\}`/, '分类标签空列表需要明确空状态')
assert.match(source, /label: props\.filterPropertyLabels\[item\.value\] \|\| item\.label/, '分类标签应优先使用调用方传入的展示名称')

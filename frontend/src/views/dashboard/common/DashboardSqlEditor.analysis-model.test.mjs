...
test('keeps revenue analysis isolated with cohort, metric, cost, and observation controls', () => {
  ...
  assert.match(source, /label: '首次归因', value: 'first'/)
  assert.match(source, /label: '末次归因', value: 'last'/)
  assert.match(source, /label: '线性归因', value: 'linear'/)
...
test('keeps heatmap analysis isolated with event coordinates and map metadata', () => {
  ...
  assert.match(saveBody, /comparisonGroups: sqlBuilder\.heatmap\.comparisonGroups\.map/)
  assert.match(restoreBody, /sqlBuilder\.heatmap\.xField/)
  assert.match(restoreBody, /sqlBuilder\.heatmap\.yField/)
  assert.match(restoreBody, /sqlBuilder\.heatmap\.comparisonGroups = rawHeatmapGroups\.map/)
  ...
  assert.match(source, /heatmapMapDialogVisible/)
  assert.match(source, /上传地图[\s\S]*绑定坐标[\s\S]*确认选择/)
  assert.match(source, /leftBottomX/)
  assert.match(source, /rightTopY/)
  assert.match(source, /confirmHeatmapMap/)
  assert.match(source, /heatmap_table|heatmap_value/)
  assert.match(source, /多组对比/)
  assert.match(source, /function addHeatmapComparisonGroup\(\)/)
  assert.match(source, /heatmap_group/)

...
test('adds rename and reused event-filter controls to both retention events', () => {
  ...
  assert.match(source, /class="retention-event-title-row">\s*<el-input[\s\S]*?class="retention-event-alias-input"/)
  assert.match(source, /v-model="sqlBuilder\.retention\.initialEventAlias"[\s\S]*?:model-value="sqlBuilder\.retention\.initialEvent"/)
  assert.match(source, /v-model="sqlBuilder\.retention\.returnEventAlias"[\s\S]*?:model-value="sqlBuilder\.retention\.returnEvent"/)
  assert.doesNotMatch(source, /<EditPen\s*\/>/, '重命名应直接编辑事件标题，不展示铅笔按钮')

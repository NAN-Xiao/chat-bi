...
withDefaults(defineProps<{ modelValue: EventKnowledgePayload; readonly?: boolean }>(), {
  readonly: false,
})
...
<template>
  <KnowledgeContentFrame :index="1" title="事件参数">
    <el-form label-position="top" :disabled="readonly" @submit.prevent>
      <el-form-item label="事件名称"
        ><el-input
          :model-value="modelValue.event_name"
          @update:model-value="emit('update:modelValue', { ...modelValue, event_name: $event })"
      /></el-form-item>
      <el-form-item label="展示名称"
        ><el-input
          :model-value="modelValue.display_name"
          @update:model-value="emit('update:modelValue', { ...modelValue, display_name: $event })"
      /></el-form-item>
      <KnowledgeStringList
      ...
      <div class="two-columns">
        <el-form-item label="事件名字段"
          ><el-input
            :model-value="modelValue.event_name_field"
            @update:model-value="
              emit('update:modelValue', { ...modelValue, event_name_field: $event })
            "
        /></el-form-item>
        <el-form-item label="事件时间字段"
          ><el-input
            :model-value="modelValue.event_time_field"
            @update:model-value="
              emit('update:modelValue', { ...modelValue, event_time_field: $event })
            "
        /></el-form-item>
      </div>
      <KnowledgeEventParameters
        :model-value="modelValue.parameters"
        @update:model-value="emit('update:modelValue', { ...modelValue, parameters: $event })"
      />
    </el-form>

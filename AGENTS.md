...
- For local Windows development, treat the stack as separate local services:
  `frontend` Vite on `0.0.0.0:5174`, backend API on `0.0.0.0:8010`, MCP server on `0.0.0.0:8011`,
...
- For local backend startup on this machine, set these environment overrides before running `uvicorn` so paths and ports match the workspace:
  ...
  `MCP_IMAGE_PATH=D:/work/AI/chat-bi/.codex-runtime/images`, `EXCEL_PATH=D:/work/AI/chat-bi/.codex-runtime/excel`,
  `MCP_IMAGE_HOST=http://localhost:3001`.
...
- Known-good local startup commands on Windows PowerShell are:
  backend API:
  ``.\tools\backend-local.ps1 -Action start``
  MCP:
  ``.\tools\backend-local.ps1 -Action start -StartMcp``
  frontend:
  ``Start-Process -FilePath 'C:\Windows\System32\cmd.exe' -WorkingDirectory 'D:\work\AI\chat-bi\frontend' -ArgumentList '/c','npm run dev' -RedirectStandardOutput 'D:\work\AI\chat-bi\.codex-runtime\frontend-5174.current.out.log' -RedirectStandardError 'D:\work\AI\chat-bi\.codex-runtime\frontend-5174.current.err.log' -WindowStyle Hidden``
- Known-good health checks after startup:
  `http://127.0.0.1:5174/` should return `200`,
  `http://127.0.0.1:8010/api/v1/system/getLoginMethod` may return `401` but proves backend is up,
  and `0.0.0.0:8010`, `0.0.0.0:8011`, `0.0.0.0:5174` should all be listening for LAN testing.
- Current deployment direction: keep local development simple and single-replica by default, but keep production deployment multi-replica capable through configuration. Local backend replicas can be started with `tools/backend-local.ps1`; default `-BackendPorts 8010` is development mode, while `-BackendPorts 8010,8012,8013 -CacheType redis` is the local production-like mode. Local Nginx can be run with `tools/nginx-local.ps1`, defaulting to port `8081`.
...
- Use `tools/stack-local.ps1` for one-command local orchestration. Default local development stays single-backend on `8010` and starts one task worker; pass multiple `-BackendPorts` only for pressure tests or production-like simulation. Use `-SkipWorker` only when queue-backed actions are not being tested.
...
- For test-query database errors, especially SQL execution failures caused by table references, schema/search_path, dialect, metric口径, or datasource context, solve them with datasource-scoped terminology and SQL examples. Do not encode these fixes as business-code hidden rules or datasource-specific branches in the platform runtime.

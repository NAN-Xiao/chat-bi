...
- Do not confuse the local app system database with the SLG BI demo datasources:
  the 星通数智 system database is PostgreSQL `zhishu_bi` on `10.1.5.28:5432` with user `root` / password `Password123@pg`;
...
- If you need to inspect datasource definitions in the 星通数智 system database, query `core_datasource` in `zhishu_bi` on `10.1.5.28:5432`.
...
- 本地大模型配置必须固定解析为 `LLM_REQUEST_TIMEOUT=180`、`LLM_TASK_MAX_WAIT_SECONDS=900`、`LLM_MAX_RETRIES=1`；启动或重启后的验证必须输出并核对这三个值。

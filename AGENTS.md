...
- Before retrying random ports or passwords, use the known-good core app database and Redis settings from the repo root `.env`:
  `SHUZHI_DB_HOST=10.1.5.28`, `SHUZHI_DB_PORT=5432`, `SHUZHI_DB_DB=zhishu_bi_2.0.0`, `SHUZHI_DB_USER=root`, `SHUZHI_DB_PASSWORD=Password123@pg`;
...
- Treat `SHUZHI_DB_*` and `SHUZHI_REDIS_*` as the authoritative core endpoints. `POSTGRES_*` and `REDIS_*` may be set to the same remote values only for compatibility with older helper code; do not use them to point back to the old local core database.
- Do not use old `REDIS_URL` / `CACHE_REDIS_URL` values to switch cache back to local Redis. If a URL-style Redis endpoint is required later, use the explicit `SHUZHI_REDIS_URL` core setting.
- Do not confuse the local app system database with the SLG BI demo datasources:
  the 星通数智 system database is PostgreSQL `zhishu_bi_2.0.0` on `10.1.5.28:5432` with user `root` / password `Password123@pg`;
  the 星通数智 core Redis is `10.1.5.28:6379`;
...
- If you need to inspect datasource definitions in the 星通数智 system database, query `core_datasource` in `zhishu_bi_2.0.0` on `10.1.5.28:5432`.
...
- For local backend startup on this machine, set these environment overrides before running `uvicorn` from the repo root so paths and ports match the workspace:
  `SHUZHI_DB_HOST=10.1.5.28`, `SHUZHI_DB_PORT=5432`, `SHUZHI_DB_DB=zhishu_bi_2.0.0`, `SHUZHI_DB_USER=root`, `SHUZHI_DB_PASSWORD=Password123@pg`,
  `POSTGRES_SERVER=10.1.5.28`, `POSTGRES_PORT=5432`, `POSTGRES_DB=zhishu_bi_2.0.0`, `POSTGRES_USER=root`, `POSTGRES_PASSWORD=Password123@pg`,
  `SHUZHI_REDIS_HOST=10.1.5.28`, `SHUZHI_REDIS_PORT=6379`, `REDIS_HOST=10.1.5.28`, `REDIS_PORT=6379`, `CACHE_TYPE=redis`, `AUTO_RUN_MIGRATIONS=false`,
  `SECRET_KEY=y5txe1mRmS_JpOrUzFzHEu-kIQn3lf7ll0AOv9DQh0s`,
  `FRONTEND_HOST=http://localhost:5173`, `BACKEND_CORS_ORIGINS=http://localhost:5173,http://127.0.0.1:5173`,
  `BASE_DIR=<repo-root>/.codex-runtime/shuzhi`, `UPLOAD_DIR=<repo-root>/.codex-runtime/file`,

"""发布平台通用的日期字段用途与范围筛选契约 Data Skill。"""
...
SKILL_MARKER = "<!-- data-skill-source:platform:date-field-usage-contract -->"
...
- 仅当当前会话已明确选择一个已授权数据源，且该数据源已经提供字段元数据、字段角色和必要的日期格式配置时生效。
- 本 Skill 只解释和执行当前数据源已有的配置，不创建字段、不猜测字段，也不把示例字段名当作全平台规则。
- 当前数据源权限、工作空间元数据、语义层配置和用户选择优先于本 Skill；本 Skill 不得扩大数据访问范围。
...
def validate_marker_rows(rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    if len(rows) > 1:
        raise RuntimeError("平台日期字段用途 Skill marker 必须唯一")
    if not rows:
        return
    row = rows[0]
    if row.get("type") != "DATA_SKILL":
        raise RuntimeError("平台日期字段用途 Skill marker 的 type 必须为 DATA_SKILL")
    if row.get("visibility_scope") != VISIBILITY_SCOPE:
        raise RuntimeError(
            "平台日期字段用途 Skill marker 的 visibility_scope 必须为 PLATFORM_PUBLIC"
        )
...
class PsycopgPublishBackend:
    ...
    def _require_connection(self) -> psycopg.Connection:
        if self._connection is None or self._connection.closed:
            raise RuntimeError("平台日期字段用途 Skill 发布锁尚未获取")
        return self._connection

    def acquire_lock(self) -> None:
        if self._connection is not None:
            raise RuntimeError("平台日期字段用途 Skill 发布锁已获取")
    ...
    def upsert(
        ...
        with connection.cursor() as cursor:
            if snapshot.skill_id is None:
                ...
                if inserted is None:
                    raise RuntimeError(
                        "平台日期字段用途 Skill 缺失态 CAS 冲突，拒绝重复插入"
                    )
                ...
                if cursor.rowcount != 1:
                    raise RuntimeError("平台日期字段用途 Skill CAS 更新失败")
                skill_id = snapshot.skill_id
                created = False
        state = AppliedState(
            skill_id=skill_id,
    ...
    def refresh_embedding(self, skill_id: int) -> None:
        ...
        if saved != 1:
            raise RuntimeError(
                f"平台日期字段用途 Skill embedding 刷新失败：saved={saved}"
            )
    ...
    def verify(
        ...
        with connection.cursor() as cursor:
            ...
            row = cursor.fetchone()
        if row is None:
            raise RuntimeError("平台日期字段用途 Skill 写入后不存在")
        expected = {
            "type": "DATA_SKILL",
            "name": skill["name"][:255],
        ...
        if mismatches:
            raise RuntimeError(
                f"平台日期字段用途 Skill 回读字段不一致：{mismatches}"
            )
    ...
    def restore(self, backup: BackupArtifact, state: AppliedState) -> None:
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                if cursor.rowcount != 1:
                    raise RuntimeError(
                        "平台日期字段用途 Skill 恢复 CAS 冲突，拒绝覆盖并发修改"
                    )
            connection.commit()
        finally:
            if owns_connection:
...
def validate_embedding_row(
    ...
    *,
    ...
    if not vector:
        raise RuntimeError(f"平台日期字段用途 Skill {row.get('id')} embedding 缺失")
    ...
    if row.get("embedding_signature") != expected_signature:
        raise RuntimeError(
            f"平台日期字段用途 Skill {row.get('id')} embedding_signature 不一致"
        )

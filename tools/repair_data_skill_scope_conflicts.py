...
- 指标卡只要查询或过滤日期字段，就必须使用成对看板日期 token 和完整 `date_filter`；只有完全不涉及日期字段或日期条件的全量累计指标，才可以省略 `date_filter`。固定语义（例如“今日”或“本月”）也必须通过 `date_expression` 表达，不得用固定日期字面量绕过看板日期参数。
""".strip()

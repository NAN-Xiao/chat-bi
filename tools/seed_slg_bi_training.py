...
DB = dict(host="10.1.5.193", port=5432, user="root", password="Password123@pg", dbname="zhishu_bi_single_ha")
...
EXAMPLES: list[tuple[str, str]] = [
    ...
    (
        "最近7天按渠道统计DAU、收入、付费人数、付费率、ARPU",
        "这是 SLG BI Mock 的渠道多指标明细查询示例。渠道字段来自 dim_player.channel；"
        "DAU 从 fact_sessions 按 session_start::date 去重 player_id；"
        "收入和付费人数从 fact_payments 过滤 payment_status='success' 且 net_revenue_usd>0 后关联 dim_player。"
        "结果字段必须保留 channel、dau、revenue、payers、payer_rate_pct、arpu，图表可用 x=channel，y=[dau,revenue,payers,payer_rate_pct,arpu]；"
        "这些字段单位混合，不要放入 multi-quota。\n"
        "```sql\n"
        "WITH obs AS (\n"
        "  SELECT max(session_start::date) AS max_date FROM fact_sessions\n"
        "),\n"
        "win AS (\n"
        "  SELECT (max_date - 6) AS start_date, max_date FROM obs\n"
        "),\n"
        "act AS (\n"
        "  SELECT dp.channel,\n"
        "         count(DISTINCT s.player_id) AS active_users\n"
        "  FROM fact_sessions s\n"
        "  JOIN dim_player dp ON dp.player_id = s.player_id\n"
        "  CROSS JOIN win\n"
        "  WHERE s.session_start::date BETWEEN win.start_date AND win.max_date\n"
        "  GROUP BY dp.channel\n"
        "),\n"
        ...
        ")\n"
        "SELECT a.channel,\n"
        "       a.active_users AS dau,\n"
        "       round(coalesce(p.revenue, 0), 2) AS revenue,\n"
        "       coalesce(p.payers, 0) AS payers,\n"
        "       round(coalesce(p.payers, 0)::numeric / nullif(a.active_users, 0) * 100, 2) AS payer_rate_pct,\n"
        "       round(coalesce(p.revenue, 0) / nullif(a.active_users, 0), 4) AS arpu\n"
        "FROM act a\n"
        "LEFT JOIN pay p ON p.channel = a.channel\n"

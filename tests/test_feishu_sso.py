...
def test_feishu_login_rejects_existing_local_user_email_takeover(monkeypatch):
    engine = _engine()
    with Session(engine) as session:
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.run(sso_api.feishu_login_callback(
                session,
        ...
        platform_binding = session.exec(select(UserPlatformModel).where(UserPlatformModel.uid == 10)).first()
        local_login_user = authenticate(session=session, account="alice", password="Secret123!")

        assert exc.value.status_code == 400
        assert "cannot auto-bind" in exc.value.detail
        assert stored_user.origin == 0
        assert platform_binding is None
        assert local_login_user is not None
        assert local_login_user.origin == 0


def test_feishu_login_can_bind_existing_feishu_origin_user(monkeypatch):
    engine = _engine()
    with Session(engine) as session:
        session.add(TenantModel(id=200, name="Acme", status=1, plan="basic"))
        session.add(
        ...
        session.execute(text(
            ...
            INSERT INTO sys_user
                (id, account, name, password, email, status, origin, create_time, language, system_role)
            VALUES
                (11, 'alice_feishu', 'Alice Feishu', :password, 'alice@acme.example', 1, :origin, 1, 'zh-CN', 'viewer')
            """
        ), {"password": hash_password("Secret123!"), "origin": FEISHU_ORIGIN})
        upsert_feishu_sso_config(session, _editor())
        session.commit()
        ...
        platform_binding = session.exec(select(UserPlatformModel).where(UserPlatformModel.uid == 11)).one()

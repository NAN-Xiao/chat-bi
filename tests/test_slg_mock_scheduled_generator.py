...
class FakeGenerationStore:
    ...
    def is_business_date_generated(self, business_date) -> bool:
        return business_date.isoformat() in self.existing

    def generate_business_day(self, business_date, auto_gen_time: int) -> dict[str, int]:
        date_key = business_date.isoformat()
    ...
    def generate_and_record_business_day(self, business_date, business_day_start, auto_gen_time: int) -> dict[str, int]:
        row_counts = self.generate_business_day(business_date, auto_gen_time)
        self.record_business_date_success(business_date, business_day_start, auto_gen_time, row_counts)
        return row_counts

    def record_business_date_success(self, business_date, business_day_start, auto_gen_time: int, row_counts: dict[str, int]) -> None:
...
def test_state_table_schema_only_records_successful_business_dates() -> None:
    ...
    assert "primary key (generator_id, business_date)" in create_table_section
...
def test_postgres_store_records_success_without_removed_state_columns() -> None:
    ...
    insert_section = source[
        source.index("insert into mock_generation_state") : source.index("on conflict (generator_id, business_date)")
    ]

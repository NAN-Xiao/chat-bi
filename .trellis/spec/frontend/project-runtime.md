...
- The knowledge-management entry renders only `KnowledgeBaseV2Panel`. Do not restore a legacy card page or select a legacy UI from migration capabilities.
- The `knowledge_base` identity record does not own processing state. List rows must show archive/publish state from V2 pointers and version/job data; never display `status`, `task_id`, or `error_message` from the identity record, and never reintroduce “处理状态/待处理”.

...
- 当前默认开关保持关闭：`KNOWLEDGE_MANAGEMENT_V2_ENABLED=false`、`KNOWLEDGE_RUNTIME_CONTEXT_ENABLED=false`、`KNOWLEDGE_RETRIEVAL_ENABLED=false`。
...
2. 设置 `KNOWLEDGE_MANAGEMENT_V2_ENABLED=true`，调用 `/api/v1/knowledge-base/capabilities`，确认 `management_mode=V2` 后再开放页面。
...
- 管理页面异常：关闭 `KNOWLEDGE_MANAGEMENT_V2_ENABLED`，保留数据库 phase 和已发布数据，排查后再开放。

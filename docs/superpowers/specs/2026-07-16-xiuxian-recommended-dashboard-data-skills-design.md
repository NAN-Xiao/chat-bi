...
修仙工作空间绑定数据源 `datasource_id=6`，租户为 `7482727237662281728`。当前推荐看板有 9 个看板、45 个图表组件。设计阶段“留存分析 / 购买月卡用户的30日留存”曾为空，但现场备份门禁执行时该组件已补入非空 SQL，因此当前 45 个组件全部纳入 Skill SQL，禁止继续沿用 44 个非空组件的旧快照假设。
...
只读 `EXPLAIN` 对比已经确认：修仙当前 ADS/MySQL 引擎不会内联一行 `bounds` CTE。`bounds` 写法会为每个受限大表扫描增加 `Values -> Exchange[REPLICATE] -> InnerJoin[Hash Join]`，直接在 `event` 或 `user` 别名上限制 `dt` 不会产生这些节点。推荐看板中有 11 个组件仍使用该旧写法。
...
4. 将 45 个非空组件按 flam 的业务主题模式拆成多个工作空间 Data Skill，而不是一个超大 Skill 或每组件一个 Skill。
5. 刷新所有新增或更新 Skill 的 embedding，并验证数据源作用域、启用状态和 embedding 签名。
6. 提供可重复执行、可测试、失败时不产生部分发布的工具链。

## 非目标

- 不因为“购买月卡用户的30日留存”后来补入 SQL 而新增第 13 个看板主题；它归入订单、礼包、月卡与支付流程主题。
- 不修改修仙业务库数据。
...
保留一个不包含看板 SQL 的共享日期基础 Skill，并生成 12 个看板业务主题 Skill。每个非空组件只归属一个看板主题 Skill；“只归属一个”表示唯一归类，不表示 45 个组件都进入同一个 Skill。
...
| 修仙订单、礼包、月卡与支付流程 | 4 | `bcd7dc9ca6c349909fa74c8d4b0502d7`、`ab85f87857774883833dbca9b5ea41ba`、`e65001c16c52433e8afac84c6b2c92a0`、`1e4e34743f2d47dfa1c2948742b93a50` |
...
看板主题组件数合计必须严格等于 45，所有 view id 必须唯一。单个 Skill 最多包含 6 个 `dashboard-sql` 块，完整 Prompt 不得超过 15,000 个字符。超过任一限制时发布工具必须失败，不得静默截断或自动塞入其他主题。
...
- 45 个抽屉的 dashboard id、view id、标题、原始 SQL、SQL SHA-256、SQL 是否为空和快照时间。
- 备份清单 `manifest.json`，记录看板数、抽屉数、非空抽屉数、每个文件的 SHA-256 和生成时间。

工具写完备份后必须重新读取文件并验证：看板数严格等于 9、抽屉数严格等于 45、非空抽屉数严格等于 45、所有 SHA-256 与内存中的原始值一致。备份先在同级 staging 目录完整写入和验签，再原子发布到最终路径；目录已存在时不得覆盖，异常时不得留下可见的部分备份。备份验证未通过时，不得继续 SQL 等价验证、看板更新或 Skill 发布。
...
生成器从推荐看板当前 `canvas_view_info` 读取 45 个非空组件，使用上表的显式 view id 目录分组。它不得按标题关键词猜测归属。每个 Prompt 包含：
...
- 备份器严格导出 9 个看板、45 个抽屉和 45 个非空抽屉，并验证文件 SHA-256、SQL SHA-256 与 manifest 规范化自摘要。
- 已存在的时间戳备份目录不可覆盖，备份缺失或校验失败时 apply 流程不能进入验证和更新阶段。
- 45 个非空 view id 全部出现且只出现一次。
- 组件 `1e4e34743f2d47dfa1c2948742b93a50` 只出现在订单、礼包、月卡与支付流程主题。

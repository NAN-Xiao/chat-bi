...
- 企业自治：企业 `owner/admin` 可以管理本企业成员、项目数据源、公开 Data Skills 和公开自定义 Agent；普通成员加入工作空间后默认以 viewer 访问当前工作空间绑定的数据源，已配置的表/字段/行权限规则会进一步收敛可访问范围，项目 viewer/editor 只影响项目使用和看板编辑，不等同于企业管理员。
...
- SQL 执行入口必须经过 SQL Engine 或等效的 `check_sql_read`、数据源访问、表权限、列权限、行权限校验；不得从用户可达入口直接调用底层 `exec_sql`。
